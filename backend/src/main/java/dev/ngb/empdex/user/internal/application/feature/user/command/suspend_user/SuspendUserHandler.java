...
public class SuspendUserHandler implements RequestHandler<SuspendUserCommand, Void> {
    private final UserRepository users;

    @Override
    public Result<Void> handle(SuspendUserCommand request) {
        var user = users.findById(request.userId());
        if (user.isEmpty()) {
            return Result.failure(UserErrorCode.USER_NOT_FOUND);
        }
        var result = user.get().suspend();
        if (result.isFailure()) {
